...
void StepControl<p, u>::delayISR(unsigned channel)
{
    ...
    if (channel == accLoopDelayChannel) // calculate new speed (i.e., timer reload value)
    {
        ...
        if (StepTimer.isRunning())
        {
            ...
            {
                motorList[0]->currentSpeed = (sqrt_2a * sqrtf(pos) + motorList[0]->initialSpeed + vMin);
                StepTimer.channel->LDVAL = F_BUS / motorList[0]->currentSpeed;
            }
            else if (pos < decelerationStart) // constant speed
            {
                StepTimer.channel->LDVAL = cMax;
                motorList[0]->currentSpeed = F_BUS / StepTimer.channel->LDVAL;
            }
            else                             //decelerating
            {
                motorList[0]->currentSpeed = (sqrt_2a * sqrtf(motorList[0]->target - pos - 1) + 0 * vMin / 2);
                StepTimer.channel->LDVAL = F_BUS / motorList[0]->currentSpeed;
            }
        }
    }
...
void StepControl<p, u>::doMove(int N, float relSpeed, bool move)
{
    ...
    motorList[0]->initialSpeed = motorList[0]->currentSpeed;

    if (v > motorList[0]->initialSpeed + vMin)
    // if (v > vMin) // acceleration required
    {
        float dv = v - motorList[0]->initialSpeed;
        //float dv = v - vMin;
        ...
        // some additional logic needed here for deceleration point - should work for rotate

        cMax = (F_BUS / v);
        sqrt_2a = sqrtf(a * 2);
        motorList[0]->currentSpeed = motorList[0]->initialSpeed + vMin;
        StepTimer.channel->LDVAL = F_BUS / motorList[0]->currentSpeed;
    }
